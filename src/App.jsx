...
// ═══════════════════════════════════════════════════════════════
// NOKTA DİZAYN v5.0 — MÜŞTERİ SUNUM + MİMARİ PROJE PORTALI
// ═══════════════════════════════════════════════════════════════
...
function Logo({sz=40,full}){
  if(full)return<div style={{display:"flex",alignItems:"center",gap:0}}>
    <svg width={sz*3.2} height={sz} viewBox="0 0 160 50">
      <text x="0" y="32" fontFamily="Arial Black,Arial,sans-serif" fontWeight="900" fontSize="30" fill="#1a3a5f">N</text>
      <text x="20" y="32" fontFamily="Arial Black,Arial,sans-serif" fontWeight="900" fontSize="30" fill="#2bb5c4">D</text>
      <rect x="42" y="6" width="6" height="6" rx="1" fill="#1a3a5f"/>
      <text x="0" y="47" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="13" letterSpacing="1" fill="#2bb5c4">COMPANIES</text>
      <text x="52" y="32" fontFamily="Arial,sans-serif" fontWeight="800" fontSize="20" fill="#1a3a5f">GROUP</text>
    </svg>
    <div style={{width:1,height:sz*0.7,background:"#8a9aaa",margin:"0 12px"}}/>
    <svg width={sz*3.6} height={sz} viewBox="0 0 180 50">
      <rect x="0" y="2" width="2" height="22" fill="#8a9aaa"/>
      <text x="6" y="20" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="16" fill="#7a8a9a">NOKTA</text>
      <text x="72" y="20" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="16" fill="#2980B9">DİZAYN</text>
      <rect x="140" y="2" width="2" height="22" fill="#2980B9"/>
      <text x="42" y="40" fontFamily="Arial Black,Arial,sans-serif" fontWeight="900" fontSize="18" fill="#5a6a7a">ND</text>
      <text x="68" y="40" fontFamily="Arial,sans-serif" fontWeight="700" fontSize="14" fill="#3a4a5a">GROUP</text>
      <rect x="110" y="28" width="4" height="4" rx="1" fill="#2980B9"/>
    </svg>
  </div>;
  return<div style={{display:"flex",alignItems:"center",gap:1}}>
    <svg width={sz} height={sz} viewBox="0 0 40 40">
      <rect width="40" height="40" rx="7" fill="#1a3a5f"/>
      <text x="4" y="27" fontFamily="Arial Black,Arial,sans-serif" fontWeight="900" fontSize="22" fill="#2bb5c4">N</text>
      <text x="19" y="27" fontFamily="Arial Black,Arial,sans-serif" fontWeight="900" fontSize="22" fill="#fff">D</text>
      <rect x="33" y="5" width="4" height="4" rx="1" fill="#2bb5c4"/>
    </svg>
  </div>
}
...
export default function App(){
  ...
  useEffect(()=>{ld("ndv5-sess").then(s=>{if(s?.email)setUser(s);setReady(true)})},[]);
  const login=async email=>{const e=email.trim().toLowerCase();if(!e)return"e";const isA=ADMINS.includes(e);const us=await ldS("ndv5-users")||[];const f=us.find(u=>u.email===e);if(!isA&&!f)return"d";const s={email:e,role:isA?"admin":"user",name:f?.name||(isA?"Yönetici":""),at:Date.now()};setUser(s);await sv("ndv5-sess",s);return"ok"};
  const logout=async()=>{setUser(null);await sv("ndv5-sess",null)};
  if(!ready)return<div style={{minHeight:"100vh",background:"#06080b",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}><Logo sz={56}/><div style={{display:"flex",alignItems:"center",gap:8,marginTop:14}}><span style={{fontSize:18,fontWeight:700,color:"#7a8a9a",letterSpacing:2}}>NOKTA</span><span style={{fontSize:18,fontWeight:700,color:"#2980B9",letterSpacing:2}}>DİZAYN</span></div><div style={{fontSize:10,color:"#5a6a7a",marginTop:4,letterSpacing:3}}>ND GROUP</div><div style={{color:"#333",fontSize:11,marginTop:12}}>v5.0 Yükleniyor...</div></div>;
...
function LoginPage({onLogin}){
  ...
  return<div style={{minHeight:"100vh",background:"#06080b",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div style={{width:"100%",maxWidth:380,textAlign:"center"}}>
      <Logo sz={56}/><div style={{display:"flex",alignItems:"center",gap:8,margin:"12px 0 2px"}}><span style={{fontSize:22,fontWeight:700,color:"#7a8a9a",letterSpacing:2}}>NOKTA</span><span style={{fontSize:22,fontWeight:700,color:"#2980B9",letterSpacing:2}}>DİZAYN</span></div>
      <p style={{fontSize:10,color:"#5a6a7a",letterSpacing:3,margin:"0 0 24px"}}>ND GROUP — MİMARİ PROJE PORTALI v5.0</p>
      <div style={{background:"#0c0f16",borderRadius:12,padding:24,border:"1px solid #161a24",textAlign:"left"}}>
        <label style={{fontSize:10,color:"#888",letterSpacing:1,display:"block",marginBottom:5}}>E-POSTA</label>
        <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="ornek@email.com" onKeyDown={e=>e.key==="Enter"&&go()} style={{width:"100%",padding:"11px 13px",background:"#080a0e",border:"1px solid #1a1e28",borderRadius:7,color:"#e0dcd4",fontSize:14,outline:"none",boxSizing:"border-box",marginBottom:4}}/>
        {err&&<div style={{fontSize:10,color:"#e74c3c",padding:"4px 0"}}>{err}</div>}
        <button onClick={go} disabled={busy} style={{width:"100%",padding:"11px",marginTop:8,background:email.trim()?"linear-gradient(135deg,#2bb5c4,#2980B9)":"#161a24",border:"none",borderRadius:7,color:email.trim()?"#06080b":"#333",fontSize:13,fontWeight:700,cursor:email.trim()?"pointer":"default"}}>{busy?"Giriş...":"GİRİŞ YAP"}</button>
      </div>
    </div>
  </div>;
}
...
function Portal({user,onLogout}){
  ...
  useEffect(()=>{(async()=>{setProjects(await ldS("ndv5-proj")||[]);setUsers(await ldS("ndv5-users")||[]);setReady(true)})()},[]);
  const saveP=async ps=>{setProjects(ps);await svS("ndv5-proj",ps)};
  const saveU=async us=>{setUsers(us);await svS("ndv5-users",us)};
  const myP=isA?projects:projects.filter(p=>p.owner===user.email);
  const goHome=()=>{setPage("dash");setActive(null);setViewMode("editor")};

  if(!ready)return<div style={{minHeight:"100vh",background:"#06080b"}}></div>;

  return<div style={{minHeight:"100vh",background:"#06080b",fontFamily:"'Segoe UI',system-ui,sans-serif",color:"#ddd"}}>
    <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"7px 12px",borderBottom:"1px solid #141820",background:"#080a0e",flexWrap:"wrap",gap:4}}>
      <div style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer"}} onClick={goHome}>
        <Logo sz={26}/><div><div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:11,fontWeight:700,color:"#7a8a9a",letterSpacing:1}}>NOKTA</span><span style={{fontSize:11,fontWeight:700,color:"#2980B9",letterSpacing:1}}>DİZAYN</span></div><div style={{fontSize:6,color:"#444",letterSpacing:2}}>ND GROUP — MİMARİ PROJE PORTALI</div></div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
        ...
        <span style={{fontSize:9,color:"#555"}}>{user.email.split("@")[0]}</span>
        <span style={{fontSize:7,padding:"1px 5px",background:isA?"#2bb5c418":"#27ae6018",borderRadius:3,color:isA?"#2bb5c4":"#27ae60"}}>{isA?"Admin":"User"}</span>
...
function NBtn({onClick,t,a}){return<button onClick={onClick} style={{padding:"4px 10px",background:a?"#2980B920":"#161a24",border:a?"1px solid #2980B955":"1px solid #1e2530",borderRadius:4,color:a?"#2980B9":"#777",fontSize:9,cursor:"pointer"}}>{t}</button>}
function MBtn({t,a,onClick}){return<button onClick={onClick} style={{padding:"3px 8px",background:a?"#2980B918":"#0c0f16",border:a?"1px solid #2980B944":"1px solid #161a24",borderRadius:3,color:a?"#2980B9":"#555",fontSize:8,cursor:"pointer"}}>{t}</button>}
...
function PresentationMode({project,onSave}){
  ...
  const box={background:"#0c0f16",borderRadius:12,padding:20,border:"1px solid #161a24",marginBottom:12};

  return<div style={{flex:1,overflow:"auto",background:"linear-gradient(180deg,#06080b 0%,#0a0e14 100%)"}}>
    {/* Slide navigation */}
    <div style={{display:"flex",justifyContent:"center",gap:4,padding:"12px 8px",borderBottom:"1px solid #141820",flexWrap:"wrap"}}>
      {slides.map((s,i)=><button key={s.id} onClick={()=>setSlide(s.id)} style={{
        padding:"6px 16px",background:slide===s.id?"#2980B922":"transparent",
        border:slide===s.id?"1px solid #2980B955":"1px solid #1e2530",
        borderRadius:20,color:slide===s.id?"#2980B9":"#555",fontSize:10,cursor:"pointer",
    ...
    <div style={{maxWidth:700,margin:"0 auto",padding:"20px 16px"}}>
      ...
      {slide==="cover"&&<div style={{textAlign:"center",padding:"40px 20px"}}>
        <div style={{display:"flex",justifyContent:"center"}}><Logo sz={50} full/></div>
        <div style={{width:60,height:1,background:"#2980B933",margin:"24px auto 20px"}}/>
        <div style={{fontSize:11,color:"#5a6a7a",letterSpacing:4,marginBottom:24}}>MİMARİ İÇ TASARIM KONSEPT SUNUMU</div>
        <div style={{fontSize:24,fontWeight:300,color:"#e0dcd4",letterSpacing:2}}>{project.name}</div>
        {project.customer&&<div style={{fontSize:14,color:"#2980B9",marginTop:8}}>Hazırlayan: Nokta Dizayn — ND Group</div>}
        {project.customer&&<div style={{fontSize:14,color:"#8a8578",marginTop:4}}>Müşteri: {project.customer}</div>}
        <div style={{fontSize:11,color:"#333",marginTop:20}}>{new Date().toLocaleDateString("tr-TR",{year:"numeric",month:"long",day:"numeric"})}</div>
        <div style={{display:"flex",justifyContent:"center",gap:20,marginTop:30}}>
          {[{l:"Alan",v:`${totalArea.toFixed(0)} m²`},{l:"Bölge",v:`${zones.length}`},{l:"Tip",v:PROJ_TYPES.find(t=>t.id===project.type)?.l}].map((s,i)=>
            <div key={i}><div style={{fontSize:20,fontWeight:600,color:"#2980B9"}}>{s.v}</div><div style={{fontSize:9,color:"#555"}}>{s.l}</div></div>)}
        </div>
        <button onClick={()=>setSlide("plan")} style={{marginTop:36,padding:"12px 36px",background:"linear-gradient(135deg,#2bb5c4,#1a3a5f)",border:"none",borderRadius:10,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",letterSpacing:2}}>SUNUMA BAŞLA →</button>
      </div>}

      {/* ─── PLAN ─── */}
      {slide==="plan"&&<div>
        <div style={{textAlign:"center",marginBottom:16}}>
          <div style={{fontSize:10,color:"#2980B9",letterSpacing:3}}>YERLEŞİM PLANI</div>
          <div style={{fontSize:18,fontWeight:300,color:"#e0dcd4",marginTop:4}}>{project.name}</div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:6}}>
          {zones.map(z=><div key={z.id} style={{padding:"8px 10px",background:z.color+"0a",border:`1px solid ${z.color}33`,borderRadius:8}}>
            <div style={{fontSize:10,fontWeight:600,color:z.color}}>{z.icon} {z.label}</div>
            <div style={{fontSize:8,color:"#555",marginTop:2}}>{((z.w*z.h)/10000).toFixed(1)} m² • {z.w/100}×{z.h/100}m</div>
          </div>)}
        </div>
      </div>}
      ...
      {slide==="3d"&&<div>
        <div style={{textAlign:"center",marginBottom:16}}>
          <div style={{fontSize:10,color:"#2980B9",letterSpacing:3}}>3D KUŞ BAKIŞI</div>
          <div style={{fontSize:18,fontWeight:300,color:"#e0dcd4",marginTop:4}}>{project.name}</div>
        </div>
      ...
      {slide==="mood"&&<div>
        <div style={{textAlign:"center",marginBottom:16}}>
          <div style={{fontSize:10,color:"#2980B9",letterSpacing:3}}>KONSEPT BOARD</div>
          <div style={{fontSize:18,fontWeight:300,color:"#e0dcd4",marginTop:4}}>Bölge Açıklamaları & Malzeme Notları</div>
        </div>
        {zones.map((z,i)=>{
            ...
            <div style={{paddingLeft:12}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div>
                  ...
                  <div style={{fontSize:9,color:"#555",marginTop:2}}>{z.w/100}m × {z.h/100}m — {((z.w*z.h)/10000).toFixed(1)} m²</div>
                </div>
                <div style={{fontSize:24,opacity:0.15}}>{z.icon}</div>
              </div>
              <div style={{marginTop:10}}>
                <div style={{fontSize:9,color:"#2980B9",letterSpacing:1,marginBottom:3}}>KONSEPT</div>
                <div style={{fontSize:11,color:"#a09a88",lineHeight:1.6}}>{mood}</div>
              </div>
              <div style={{marginTop:10}}>
                <div style={{fontSize:9,color:"#8B4513",letterSpacing:1,marginBottom:3}}>MALZEME & DETAY</div>
                <div style={{fontSize:11,color:"#8a8578",lineHeight:1.6}}>{mat}</div>
              </div>
              {z.notes&&<div style={{marginTop:10}}>
                <div style={{fontSize:9,color:"#7F8C8D",letterSpacing:1,marginBottom:3}}>NOT</div>
                <div style={{fontSize:11,color:"#6b6560",lineHeight:1.6}}>{z.notes}</div>
              </div>}
            </div>
          </div>
        })}
      ...
      {slide==="walk"&&<div>
        <div style={{textAlign:"center",marginBottom:16}}>
          <div style={{fontSize:10,color:"#2980B9",letterSpacing:3}}>MEKAN TURU</div>
          <div style={{fontSize:18,fontWeight:300,color:"#e0dcd4",marginTop:4}}>Adım Adım Bölge Gezisi</div>
        </div>
        {zones.length>0&&<>
          {/* Progress bar */}
          <div style={{display:"flex",gap:3,marginBottom:16}}>
            {zones.map((z,i)=><div key={z.id} style={{flex:1,height:4,borderRadius:2,background:i<=walkIdx?z.color:"#161a24",transition:"all 0.3s"}}/>)}
          </div>
          {(()=>{
            ...
            return<div style={{...box,borderColor:z.color+"44",textAlign:"center",padding:30}}>
              ...
              <div style={{fontSize:10,color:"#555",letterSpacing:3}}>DURAK {walkIdx+1} / {zones.length}</div>
              ...
              <div style={{fontSize:12,color:"#a09a88",lineHeight:1.8,maxWidth:480,margin:"0 auto"}}>
                {z.mood||preset?.mood||"Bu bölge projenin önemli bir parçası."}
              </div>
              <div style={{marginTop:14,fontSize:11,color:"#666",lineHeight:1.6}}>
                <span style={{color:"#8B4513"}}>Malzeme:</span> {z.material||preset?.material||"Belirlenecek"}
              </div>
              <div style={{marginTop:8,display:"flex",justifyContent:"center",gap:16,fontSize:10,color:"#444"}}>
                <span>📐 {z.w/100}m × {z.h/100}m</span>
                <span>📏 {((z.w*z.h)/10000).toFixed(1)} m²</span>
              </div>
              {z.notes&&<div style={{marginTop:10,fontSize:10,color:"#6b6560",fontStyle:"italic"}}>📝 {z.notes}</div>}
          ...
          <div style={{display:"flex",justifyContent:"center",gap:10,marginTop:12}}>
            <button onClick={()=>setWalkIdx(Math.max(0,walkIdx-1))} disabled={walkIdx===0} style={{padding:"10px 24px",background:walkIdx>0?"#161a24":"#0c0f16",border:"1px solid #1e2530",borderRadius:8,color:walkIdx>0?"#2980B9":"#333",fontSize:12,cursor:walkIdx>0?"pointer":"default"}}>← Önceki</button>
            <button onClick={()=>setWalkIdx(Math.min(zones.length-1,walkIdx+1))} disabled={walkIdx>=zones.length-1} style={{padding:"10px 24px",background:walkIdx<zones.length-1?"linear-gradient(135deg,#2bb5c4,#2980B9)":"#0c0f16",border:"none",borderRadius:8,color:walkIdx<zones.length-1?"#06080b":"#333",fontSize:12,fontWeight:700,cursor:walkIdx<zones.length-1?"pointer":"default"}}>Sonraki →</button>
          </div>
        </>}
      </div>}
      ...
      {slide==="approve"&&<div style={{textAlign:"center",padding:"30px 20px"}}>
        <div style={{fontSize:10,color:"#2980B9",letterSpacing:3,marginBottom:16}}>KONSEPT ONAY</div>
        <div style={{...box,maxWidth:480,margin:"0 auto"}}>
          <div style={{fontSize:16,fontWeight:300,color:"#e0dcd4",marginBottom:8}}>{project.name}</div>
          {project.customer&&<div style={{fontSize:12,color:"#8a8578",marginBottom:16}}>Müşteri: {project.customer}</div>}
          <div style={{display:"flex",justifyContent:"center",gap:16,marginBottom:20}}>
            <div><div style={{fontSize:18,fontWeight:600,color:"#2980B9"}}>{totalArea.toFixed(0)}m²</div><div style={{fontSize:8,color:"#555"}}>Alan</div></div>
            <div><div style={{fontSize:18,fontWeight:600,color:"#2980B9"}}>{zones.length}</div><div style={{fontSize:8,color:"#555"}}>Bölge</div></div>
          </div>
          <div style={{width:"100%",height:1,background:"#1e2530",marginBottom:20}}/>

          {project.approvedAt?<div>
            <div style={{fontSize:40,marginBottom:8}}>✅</div>
            <div style={{fontSize:14,fontWeight:600,color:"#27AE60"}}>KONSEPT ONAYLANDI</div>
            <div style={{fontSize:10,color:"#888",marginTop:6}}>{new Date(project.approvedAt).toLocaleString("tr-TR")}</div>
            {project.approvedBy&&<div style={{fontSize:10,color:"#666",marginTop:2}}>Onaylayan: {project.approvedBy}</div>}
          </div>:<div>
            <div style={{fontSize:12,color:"#8a8578",lineHeight:1.8,marginBottom:20}}>
            ...
            <div style={{marginBottom:12}}>
              <input id="approverName" placeholder="Onaylayan adı (opsiyonel)" style={{width:"100%",maxWidth:300,padding:"8px 12px",background:"#080a0e",border:"1px solid #1a1e28",borderRadius:6,color:"#e0dcd4",fontSize:12,outline:"none",boxSizing:"border-box",textAlign:"center"}}/>
            </div>
            <button onClick={()=>{
            ...
            }} style={{padding:"14px 40px",background:"linear-gradient(135deg,#27AE60,#1E8449)",border:"none",borderRadius:10,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",letterSpacing:1}}>
              ✅ KONSEPTİ ONAYLIYORUM
            </button>
            <div style={{fontSize:8,color:"#333",marginTop:10}}>Bu işlem tarih damgası ile kaydedilecektir</div>
          </div>}
        </div>
        {project.approvedAt&&<div style={{marginTop:20}}>
          <div style={{fontSize:10,color:"#888",marginBottom:8}}>Onay sonrası süreç:</div>
          <div style={{display:"flex",justifyContent:"center",gap:6}}>
            {STATUS_FLOW.filter(s=>s.phase!=="sunum").map(s=><div key={s.id} style={{padding:"6px 12px",background:s.color+"15",border:`1px solid ${s.color}33`,borderRadius:6,fontSize:9,color:s.color}}>{s.icon} {s.label}</div>)}
          </div>
        </div>}
      </div>}
    </div>
...
function PlanSVG({project,zones,walls,highlightId,showLabels=true,showDims,small}){
  ...
  return<svg viewBox={`0 0 ${svgW} ${svgH}`} style={{width:"100%",maxWidth:small?300:600,display:"block",margin:"0 auto"}}>
    <defs><pattern id="pg" width={cm(100)} height={cm(100)} patternUnits="userSpaceOnUse"><path d={`M ${cm(100)} 0 L 0 0 0 ${cm(100)}`} fill="none" stroke="#0e1218" strokeWidth="0.3"/></pattern></defs>
    <rect x={PAD} y={PAD} width={cm(W)} height={cm(H)} fill="url(#pg)"/>
    <rect x={PAD} y={PAD} width={cm(W)} height={cm(H)} fill="none" stroke={small?"#1e2530":"#2a3545"} strokeWidth={small?2:3} rx={1}/>
    {/* Entrance */}
    {(()=>{const e=project.entrance||{side:"bottom",position:50,widthCm:200};const eW=cm(e.widthCm);let ex,ey,ew,eh;if(e.side==="bottom"){ex=PAD+cm(W)*(e.position/100)-eW/2;ey=PAD+cm(H)-2;ew=eW;eh=4}else if(e.side==="top"){ex=PAD+cm(W)*(e.position/100)-eW/2;ey=PAD-2;ew=eW;eh=4}else if(e.side==="left"){ex=PAD-2;ey=PAD+cm(H)*(e.position/100)-eW/2;ew=4;eh=eW}else{ex=PAD+cm(W)-2;ey=PAD+cm(H)*(e.position/100)-eW/2;ew=4;eh=eW}return<><rect x={ex} y={ey} width={ew} height={eh} fill="#06080b"/>{!small&&<text x={e.side==="bottom"?ex+ew/2:ex+10} y={e.side==="bottom"?ey+12:ey+eh/2} textAnchor="middle" fill="#2980B9" fontSize={small?4:6}>GİRİŞ</text>}</>})()}
    {showDims&&<><text x={PAD+cm(W)/2} y={PAD-8} textAnchor="middle" fill="#2a3545" fontSize={6}>{W}cm ({(W/100).toFixed(1)}m)</text><text x={PAD-10} y={PAD+cm(H)/2} textAnchor="middle" fill="#2a3545" fontSize={6} transform={`rotate(-90,${PAD-10},${PAD+cm(H)/2})`}>{H}cm</text></>}
    ...
    {zones.map(z=>{const isHL=highlightId===z.id;const rx=PAD+cm(z.x),ry=PAD+cm(z.y),rw=cm(z.w),rh=cm(z.h);return<g key={z.id}>
      <rect x={rx} y={ry} width={rw} height={rh} fill={z.color+(isHL?"40":"18")} stroke={z.color+(isHL?"ee":"55")} strokeWidth={isHL?2.5:0.7} rx={2}/>
      {showLabels&&<><text x={rx+rw/2} y={ry+rh/2-1} textAnchor="middle" fill={z.color} fontSize={Math.min(small?5:6.5,rw/(small?6:8))} fontWeight="bold" style={{pointerEvents:"none"}}>{z.icon} {z.label}</text>{!small&&<text x={rx+rw/2} y={ry+rh/2+6} textAnchor="middle" fill={z.color+"66"} fontSize={Math.min(4,rw/13)} style={{pointerEvents:"none"}}>{((z.w*z.h)/10000).toFixed(1)}m²</text>}</>}
    </g>})}
  </svg>;
}
...
function Iso3D({project}){
  ...
  return<svg width="100%" viewBox="0 0 600 400" style={{display:"block",maxWidth:600,margin:"0 auto"}}>
    <polygon points={`${iX(0,0)},${iY(0,0)} ${iX(W/20,0)},${iY(W/20,0)} ${iX(W/20,H/20)},${iY(W/20,H/20)} ${iX(0,H/20)},${iY(0,H/20)}`} fill="#0e1218" stroke="#2a3545" strokeWidth={1}/>
    {Array.from({length:Math.floor(W/200)+1}).map((_,i)=><line key={`gv${i}`} x1={iX(i*10,0)} y1={iY(i*10,0)} x2={iX(i*10,H/20)} y2={iY(i*10,H/20)} stroke="#141820" strokeWidth={0.3}/>)}
    {Array.from({length:Math.floor(H/200)+1}).map((_,i)=><line key={`gh${i}`} x1={iX(0,i*10)} y1={iY(0,i*10)} x2={iX(W/20,i*10)} y2={iY(W/20,i*10)} stroke="#141820" strokeWidth={0.3}/>)}
...
function Dash({projects,all,isA,user,onOpen,onNew,onTpl,onDel}){
  ...
  return<div style={{maxWidth:920,margin:"0 auto",padding:"16px 12px"}}>
    ...
    <div style={{display:"flex",gap:4,marginBottom:10,overflowX:"auto",paddingBottom:4}}>
      <div onClick={()=>setSFilt("all")} style={{padding:"6px 12px",background:sFilt==="all"?"#2980B915":"#0c0f16",border:`1px solid ${sFilt==="all"?"#2980B944":"#161a24"}`,borderRadius:7,cursor:"pointer",minWidth:60,textAlign:"center",flexShrink:0}}>
        <div style={{fontSize:16,fontWeight:700,color:"#2980B9"}}>{projects.length}</div><div style={{fontSize:7,color:"#555"}}>Tümü</div>
      </div>
      {STATUS_FLOW.map(s=>{const c=projects.filter(p=>p.status===s.id).length;return<div key={s.id} onClick={()=>setSFilt(sFilt===s.id?"all":s.id)} style={{padding:"6px 10px",background:sFilt===s.id?s.color+"22":"#0c0f16",border:`1px solid ${sFilt===s.id?s.color+"55":"#161a24"}`,borderRadius:7,cursor:"pointer",minWidth:55,textAlign:"center",flexShrink:0}}>
        <div style={{fontSize:14,fontWeight:700,color:s.color}}>{c}</div><div style={{fontSize:7,color:"#555"}}>{s.icon}{s.label}</div>
      </div>})}
    </div>

    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:6}}>
      <h2 style={{margin:0,fontSize:16,fontWeight:300,color:"#e0dcd4",letterSpacing:2}}>{isA?"Tüm Projeler":"Projelerim"}</h2>
      <div style={{display:"flex",gap:4}}>
        <button onClick={onTpl} style={{padding:"7px 14px",background:"#161a24",border:"1px solid #1e2530",borderRadius:6,color:"#2980B9",fontSize:10,cursor:"pointer"}}>📂 Şablon</button>
        <button onClick={onNew} style={{padding:"7px 16px",background:"linear-gradient(135deg,#2bb5c4,#2980B9)",border:"none",borderRadius:6,color:"#fff",fontSize:10,fontWeight:700,cursor:"pointer"}}>+ Yeni</button>
      </div>
    </div>
    ...
    {shown.length===0?<div style={{textAlign:"center",padding:"40px",border:"2px dashed #161a24",borderRadius:14}}><div style={{fontSize:36,marginBottom:8}}>📐</div><div style={{fontSize:13,color:"#444"}}>Proje bulunamadı</div></div>
    :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:8}}>
      {shown.map(p=>{const st=STATUS_FLOW.find(s=>s.id===p.status)||STATUS_FLOW[0];return<div key={p.id} style={{background:"#0c0f16",border:"1px solid #161a24",borderRadius:9,padding:12,cursor:"pointer",position:"relative"}} onClick={()=>onOpen(p)}>
        <div style={{position:"absolute",top:7,right:7,display:"flex",gap:3}}>
          <span style={{fontSize:7,padding:"2px 5px",background:st.color+"18",borderRadius:3,color:st.color}}>{st.icon}{st.label}</span>
          {p.approvedAt&&<span style={{fontSize:7,padding:"2px 5px",background:"#27AE6018",borderRadius:3,color:"#27AE60"}}>✅</span>}
          <button onClick={e=>{e.stopPropagation();setCfm(p.id)}} style={{background:"none",border:"none",color:"#333",cursor:"pointer",fontSize:10}}>✕</button>
        </div>
        {cfm===p.id&&<div onClick={e=>e.stopPropagation()} style={{position:"absolute",inset:0,background:"#0c0f16ee",borderRadius:9,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6,zIndex:5}}>
          <div style={{fontSize:9,color:"#e74c3c"}}>Silmek istediğinize emin misiniz?</div>
          <div style={{display:"flex",gap:4}}><button onClick={()=>{onDel(p.id);setCfm(null)}} style={{padding:"4px 12px",background:"#e74c3c",border:"none",borderRadius:4,color:"#fff",fontSize:9,cursor:"pointer"}}>Sil</button><button onClick={()=>setCfm(null)} style={{padding:"4px 12px",background:"#161a24",border:"none",borderRadius:4,color:"#666",fontSize:9,cursor:"pointer"}}>İptal</button></div>
        </div>}
        <div style={{fontSize:11,fontWeight:600,color:"#e0dcd4",marginBottom:3,paddingRight:80}}>{p.name}</div>
        {p.customer&&<div style={{fontSize:8,color:"#2980B9",marginBottom:3}}>👤 {p.customer}</div>}
        <div style={{display:"flex",gap:6,fontSize:8,color:"#444",flexWrap:"wrap"}}>
...
function TplPick({user,onCancel,onCreate}){
  ...
  return<div style={{maxWidth:560,margin:"0 auto",padding:"20px 14px"}}>
    <h2 style={{fontSize:16,fontWeight:300,color:"#e0dcd4",letterSpacing:2,marginBottom:12}}>Şablondan Proje</h2>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{TEMPLATES.map(t=><div key={t.id} onClick={()=>use(t)} style={{background:"#0c0f16",border:"1px solid #161a24",borderRadius:10,padding:14,cursor:"pointer",textAlign:"center"}}><div style={{fontSize:24}}>{PROJ_TYPES.find(p=>p.id===t.type)?.i}</div><div style={{fontSize:11,fontWeight:600,color:"#e0dcd4",marginTop:4}}>{t.name}</div><div style={{fontSize:9,color:"#555",marginTop:2}}>{t.zones.length} bölge</div></div>)}</div>
    <button onClick={onCancel} style={{marginTop:12,padding:"7px 18px",background:"none",border:"1px solid #161a24",borderRadius:6,color:"#555",fontSize:10,cursor:"pointer"}}>İptal</button>
  </div>;
}
...
function NewProj({user,isA,users,onCancel,onCreate}){
  ...
  return<div style={{maxWidth:480,margin:"0 auto",padding:"18px 14px"}}>
    <h2 style={{fontSize:16,fontWeight:300,color:"#e0dcd4",letterSpacing:2,marginBottom:12}}>Yeni Proje</h2>
    ...
    <Fld l="TİP"><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:4}}>{PROJ_TYPES.map(tp=><button key={tp.id} onClick={()=>setT(tp.id)} style={{padding:"7px 3px",background:t===tp.id?"#2980B915":"#0c0f16",border:t===tp.id?"1px solid #2980B944":"1px solid #161a24",borderRadius:6,cursor:"pointer",textAlign:"center"}}><div style={{fontSize:15}}>{tp.i}</div><div style={{fontSize:7,color:t===tp.id?"#2980B9":"#444",marginTop:2}}>{tp.l}</div></button>)}</div></Fld>
    <Fld l="ÖLÇÜ"><div style={{display:"flex",gap:6}}><input type="number" value={w} onChange={e=>setW(+e.target.value)} style={{...inp,flex:1}}/><input type="number" value={h} onChange={e=>setH(+e.target.value)} style={{...inp,flex:1}}/><div style={{padding:"7px 10px",background:"#141820",borderRadius:5,fontSize:11,color:"#2980B9",fontWeight:600,alignSelf:"center"}}>{((w*h)/10000).toFixed(0)}m²</div></div></Fld>
    <Fld l="GİRİŞ"><div style={{display:"flex",gap:4}}>{[{id:"bottom",l:"Alt"},{id:"top",l:"Üst"},{id:"left",l:"Sol"},{id:"right",l:"Sağ"}].map(s=><button key={s.id} onClick={()=>setEs(s.id)} style={{flex:1,padding:"6px",background:es===s.id?"#2980B915":"#0c0f16",border:es===s.id?"1px solid #2980B944":"1px solid #161a24",borderRadius:4,cursor:"pointer",fontSize:9,color:es===s.id?"#2980B9":"#444"}}>{s.l}</button>)}</div></Fld>
    <div style={{display:"flex",gap:6,justifyContent:"flex-end",marginTop:4}}>
      <button onClick={onCancel} style={{padding:"8px 18px",background:"none",border:"1px solid #161a24",borderRadius:6,color:"#555",fontSize:10,cursor:"pointer"}}>İptal</button>
      <button onClick={go} disabled={!n.trim()} style={{padding:"8px 24px",background:n.trim()?"linear-gradient(135deg,#2bb5c4,#2980B9)":"#161a24",border:"none",borderRadius:6,color:n.trim()?"#06080b":"#333",fontSize:11,fontWeight:700,cursor:n.trim()?"pointer":"default"}}>OLUŞTUR</button>
    </div>
  </div>;
}

function Fld({l,children}){return<div style={{marginBottom:10}}><label style={{fontSize:9,color:"#666",letterSpacing:1,display:"block",marginBottom:3}}>{l}</label>{children}</div>}
const inp={width:"100%",padding:"8px 10px",background:"#0c0f16",border:"1px solid #161a24",borderRadius:6,color:"#e0dcd4",fontSize:12,outline:"none",boxSizing:"border-box"};
...
function UserMgmt({users,projects,onSave}){
  ...
  return<div style={{maxWidth:600,margin:"0 auto",padding:"18px 14px"}}>
    <h2 style={{fontSize:16,fontWeight:300,color:"#e0dcd4",letterSpacing:2,marginBottom:14}}>Kullanıcı Yönetimi</h2>
    <div style={{background:"#0c0f16",borderRadius:9,padding:12,border:"1px solid #161a24",marginBottom:10}}>
      <div style={{fontSize:9,color:"#2980B9",letterSpacing:2,marginBottom:6}}>YÖNETİCİLER</div>
      {ADMINS.map(a=><div key={a} style={{padding:"4px 0",fontSize:10,color:"#ddd",borderBottom:"1px solid #111418"}}>🔑 {a}</div>)}
    </div>
    <div style={{background:"#0c0f16",borderRadius:9,padding:12,border:"1px solid #161a24",marginBottom:10}}>
      <div style={{fontSize:9,color:"#27ae60",letterSpacing:2,marginBottom:6}}>DAVET ET</div>
      <div style={{display:"flex",gap:4}}><input value={nm} onChange={e=>setNm(e.target.value)} placeholder="Ad" style={{...inp,flex:1}}/><input value={em} onChange={e=>setEm(e.target.value)} placeholder="E-posta" onKeyDown={e=>e.key==="Enter"&&inv()} style={{...inp,flex:2}}/><button onClick={inv} style={{padding:"7px 14px",background:"#27ae60",border:"none",borderRadius:5,color:"#fff",fontSize:10,cursor:"pointer",whiteSpace:"nowrap"}}>+Davet</button></div>
    </div>
    <div style={{background:"#0c0f16",borderRadius:9,padding:12,border:"1px solid #161a24"}}>
      <div style={{fontSize:9,color:"#5DADE2",letterSpacing:2,marginBottom:6}}>KULLANICILAR ({users.length})</div>
      {users.map(u=><div key={u.email} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:"1px solid #111418"}}>
        <div><div style={{fontSize:10,color:"#ddd"}}>{u.name}</div><div style={{fontSize:8,color:"#444"}}>{u.email}</div></div>
        <button onClick={()=>onSave(users.filter(x=>x.email!==u.email))} style={{background:"none",border:"none",color:"#e74c3c44",cursor:"pointer",fontSize:10}}>✕</button>
      </div>)}
    </div>
  </div>;
}
...
function Editor({project,user,onSave}){
  ...
  return<div style={{display:"flex",height:"calc(100vh - 49px)",overflow:"hidden"}}>
    <div style={{width:255,minWidth:190,background:"#080a0e",borderRight:"1px solid #141820",display:"flex",flexDirection:"column"}}>
      <div style={{display:"flex",borderBottom:"1px solid #141820",flexShrink:0,flexWrap:"wrap"}}>
        {[{id:"zones",l:"Bölge"},{id:"add",l:"+Ekle"},{id:"draw",l:"Çizim"},{id:"equip",l:"Ekipman"},{id:"layers",l:"Katman"}].map(t2=>
          <button key={t2.id} onClick={()=>setTab(t2.id)} style={{flex:1,padding:"6px 1px",background:"none",border:"none",borderBottom:tab===t2.id?"2px solid #2bb5c4":"2px solid transparent",color:tab===t2.id?"#2980B9":"#555",fontSize:8,cursor:"pointer",minWidth:36}}>{t2.l}</button>)}
      </div>
      <div style={{flex:1,padding:6,overflowY:"auto",fontSize:10}}>
        {tab==="zones"&&<>{zones.map(z=><div key={z.id} onClick={()=>setSelId(selId===z.id?null:z.id)} style={{padding:"4px 6px",borderRadius:5,marginBottom:2,background:selId===z.id?z.color+"15":"transparent",border:selId===z.id?`1px solid ${z.color}33`:"1px solid transparent",cursor:"pointer"}}>
          <div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontWeight:600,color:"#ddd"}}><span style={{color:z.color}}>■</span>{z.icon}{z.label}</span><button onClick={e=>{e.stopPropagation();delZone(z.id)}} style={{background:"none",border:"none",color:"#333",cursor:"pointer",fontSize:8}}>✕</button></div>
          <div style={{fontSize:7,color:"#444"}}>{z.w}×{z.h} • {(z.equip||[]).length}eq</div>
        </div>)}
        {sel&&<div style={{borderTop:"1px solid #141820",paddingTop:5,marginTop:4}}>
          <div style={{fontSize:9,color:sel.color,fontWeight:600,marginBottom:3}}>{sel.icon}{sel.label}</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:3,marginBottom:3}}>
            {[{l:"X",k:"x"},{l:"Y",k:"y"},{l:"W",k:"w"},{l:"H",k:"h"}].map(f=><div key={f.k}><div style={{fontSize:6,color:"#333"}}>{f.l}</div><input type="number" value={sel[f.k]} onChange={e=>updZone(sel.id,{[f.k]:+e.target.value})} style={{width:"100%",padding:"2px 3px",background:"#0c0f16",border:"1px solid #161a24",borderRadius:2,color:"#ddd",fontSize:9,outline:"none",boxSizing:"border-box"}}/></div>)}
          </div>
          <div style={{fontSize:7,color:"#444",marginBottom:1}}>Konsept notu</div>
          <textarea value={sel.mood||""} onChange={e=>updZone(sel.id,{mood:e.target.value})} placeholder="Konsept açıklama..." style={{width:"100%",padding:"3px",background:"#0c0f16",border:"1px solid #161a24",borderRadius:2,color:"#ddd",fontSize:8,outline:"none",minHeight:24,resize:"vertical",boxSizing:"border-box",fontFamily:"inherit"}}/>
          <div style={{fontSize:7,color:"#444",marginTop:3,marginBottom:1}}>Malzeme</div>
          <textarea value={sel.material||""} onChange={e=>updZone(sel.id,{material:e.target.value})} placeholder="Malzeme detayları..." style={{width:"100%",padding:"3px",background:"#0c0f16",border:"1px solid #161a24",borderRadius:2,color:"#ddd",fontSize:8,outline:"none",minHeight:24,resize:"vertical",boxSizing:"border-box",fontFamily:"inherit"}}/>
          <div style={{fontSize:7,color:"#2980B9",marginTop:3}}>Ekipman: {(sel.equip||[]).length}</div>
          {(sel.equip||[]).map(eq=><div key={eq.uid} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",fontSize:8,borderBottom:"1px solid #0e1118"}}>
            <span style={{color:"#666"}}>{eq.brand} {eq.name}</span>
            <button onClick={()=>updZone(sel.id,{equip:(sel.equip||[]).filter(e=>e.uid!==eq.uid)})} style={{background:"none",border:"none",color:"#333",cursor:"pointer",fontSize:7}}>✕</button>
          </div>)}
        </div>}
        </>}
        {tab==="add"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:3}}>{ZONE_PRESETS.map(t2=><button key={t2.id+Math.random()} onClick={()=>addZone(t2)} style={{padding:"6px 2px",background:"#0c0f16",border:"1px solid #161a24",borderRadius:5,cursor:"pointer",textAlign:"center"}}><div style={{fontSize:14}}>{t2.icon}</div><div style={{fontSize:7,color:"#666",marginTop:1}}>{t2.label}</div></button>)}</div>}
        {tab==="draw"&&<div>
          {[{id:"wall",l:"🧱 Duvar",c:"#4a5568"},{id:"elec",l:"⚡ Elektrik",c:"#F1C40F"},{id:"plumb",l:"💧 Tesisat",c:"#3498DB"},{id:"measure",l:"📏 Ölçü",c:"#E67E22"}].map(d=>
            <button key={d.id} onClick={()=>{setDrawMode(drawMode===d.id?null:d.id);setDrawStart(null)}} style={{display:"block",width:"100%",padding:"5px 7px",marginBottom:2,background:drawMode===d.id?d.c+"22":"#0c0f16",border:drawMode===d.id?`1px solid ${d.c}55`:"1px solid #161a24",borderRadius:4,color:drawMode===d.id?d.c:"#666",fontSize:9,cursor:"pointer",textAlign:"left"}}>{d.l}{drawMode===d.id?" ✓":""}</button>)}
          ...
          {drawStart&&<div style={{marginTop:4,fontSize:8,color:"#2980B9",padding:3,background:"#2980B911",borderRadius:3}}>Nokta 1: ({drawStart.x},{drawStart.y})</div>}
        </div>}
        {tab==="equip"&&<div>
          {!sel&&<div style={{padding:5,background:"#2980B911",borderRadius:4,fontSize:8,color:"#2980B9",marginBottom:4}}>Önce bölge seçin</div>}
          ...
          {EQUIPMENT[eqCat]?.items.map(eq=><div key={eq.id} style={{padding:"4px 5px",marginBottom:2,background:"#0c0f16",borderRadius:4,border:"1px solid #161a24"}}>
            <div style={{fontSize:9,fontWeight:600,color:"#ddd"}}>{eq.name}</div>
            <div style={{fontSize:7,color:"#555"}}><span style={{color:eq.color}}>{eq.brand}</span> • {eq.model}</div>
            <div style={{fontSize:7,color:"#444",display:"flex",justifyContent:"space-between",marginTop:1}}>
              <span>{eq.power>0?eq.power+"W":"—"} • ₺{fmt(eq.price)}</span>
              {sel&&<button onClick={()=>updZone(sel.id,{equip:[...(sel.equip||[]),{...eq,uid:Date.now().toString()}]})} style={{padding:"1px 6px",background:"#2980B922",border:"1px solid #2980B944",borderRadius:2,color:"#2980B9",fontSize:7,cursor:"pointer"}}>+Ekle</button>}
            </div>
          </div>)}
        </div>}
        {tab==="layers"&&<div>
          {[{k:"zones",l:"Bölgeler",c:"#2980B9"},{k:"walls",l:"Duvarlar",c:"#4a5568"},{k:"equip",l:"Ekipmanlar",c:"#27ae60"},{k:"elec",l:"Elektrik",c:"#F1C40F"},{k:"plumb",l:"Tesisat",c:"#3498DB"},{k:"measure",l:"Ölçüler",c:"#E67E22"}].map(ly=>
            <label key={ly.k} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 3px",cursor:"pointer",fontSize:10,color:layers[ly.k]?ly.c:"#333"}}>
              <input type="checkbox" checked={layers[ly.k]} onChange={()=>setLayers(l=>({...l,[ly.k]:!l[ly.k]}))} style={{accentColor:ly.c}}/>{ly.l}
            </label>)}
        </div>}
      </div>
      <div style={{padding:5,borderTop:"1px solid #141820",flexShrink:0}}>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:7,color:"#444",marginBottom:3}}>
        ...
        <div style={{display:"flex",gap:2,flexWrap:"wrap"}}>
          {[{id:"report",t:"📋Rapor"},{id:"pricing",t:"💰Teklif"},{id:"energy",t:"⚡Enerji"},{id:"status",t:"📊Durum"}].map(b=><button key={b.id} onClick={()=>setPanel(panel===b.id?null:b.id)} style={{padding:"3px 6px",background:"#161a24",border:"1px solid #1e2530",borderRadius:3,color:"#888",fontSize:7,cursor:"pointer"}}>{b.t}</button>)}
          <button onClick={saveVer} style={{padding:"3px 6px",background:"#161a24",border:"1px solid #1e2530",borderRadius:3,color:"#888",fontSize:7,cursor:"pointer"}}>💾Rev</button>
        </div>
      </div>
    </div>

    <div style={{flex:1,overflow:"auto",background:"#06080b",position:"relative"}}>
      ...
      <div style={{padding:8,display:"flex",justifyContent:"center"}}>
        <svg ref={svgRef} viewBox={`0 0 ${svgW} ${svgH}`} style={{width:"100%",maxWidth:700,display:"block",touchAction:"none",cursor:drawMode?"crosshair":"default"}} onClick={canvasClick}>
          <defs><pattern id="g1" width={cm(100)} height={cm(100)} patternUnits="userSpaceOnUse"><path d={`M ${cm(100)} 0 L 0 0 0 ${cm(100)}`} fill="none" stroke="#0c0f16" strokeWidth="0.3"/></pattern><filter id="gl"><feGaussianBlur stdDeviation="2" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
          <rect x={PAD} y={PAD} width={cm(W)} height={cm(H)} fill="url(#g1)"/><rect x={PAD} y={PAD} width={cm(W)} height={cm(H)} fill="none" stroke="#2a3545" strokeWidth={3} rx={1}/>
          {(()=>{const e=project.entrance||{side:"bottom",position:50,widthCm:200};const eW=cm(e.widthCm);let ex,ey,ew,eh;if(e.side==="bottom"){ex=PAD+cm(W)*(e.position/100)-eW/2;ey=PAD+cm(H)-2;ew=eW;eh=5}else if(e.side==="top"){ex=PAD+cm(W)*(e.position/100)-eW/2;ey=PAD-2;ew=eW;eh=5}else if(e.side==="left"){ex=PAD-2;ey=PAD+cm(H)*(e.position/100)-eW/2;ew=5;eh=eW}else{ex=PAD+cm(W)-2;ey=PAD+cm(H)*(e.position/100)-eW/2;ew=5;eh=eW}return<><rect x={ex} y={ey} width={ew} height={eh} fill="#06080b"/><text x={e.side==="bottom"?ex+ew/2:ex+10} y={e.side==="bottom"?ey+14:ey+eh/2} textAnchor="middle" fill="#2980B9" fontSize={6}>GİRİŞ</text></>})()}
          <text x={PAD+cm(W)/2} y={PAD-10} textAnchor="middle" fill="#2a3545" fontSize={6}>{W}cm</text>
          <text x={PAD-12} y={PAD+cm(H)/2} textAnchor="middle" fill="#2a3545" fontSize={6} transform={`rotate(-90,${PAD-12},${PAD+cm(H)/2})`}>{H}cm</text>
          ...
          {layers.zones&&zones.map(z=>{const isSel=selId===z.id;const rx=PAD+cm(z.x),ry=PAD+cm(z.y),rw=cm(z.w),rh=cm(z.h);return<g key={z.id}>
            <rect x={rx} y={ry} width={rw} height={rh} fill={z.color+(isSel?"28":"14")} stroke={z.color+(isSel?"cc":"55")} strokeWidth={isSel?2:0.7} rx={2} filter={isSel?"url(#gl)":"none"} onMouseDown={e=>onPD(e,z.id)} onTouchStart={e=>onPD(e,z.id)} style={{cursor:drawMode?"crosshair":"grab"}}/>
            ...
            <text x={rx+rw/2} y={ry+rh/2-1} textAnchor="middle" fill={z.color} fontSize={Math.min(6,rw/9)} fontWeight="bold" style={{pointerEvents:"none"}}>{z.icon}{z.label}</text>
            <text x={rx+rw/2} y={ry+rh/2+6} textAnchor="middle" fill={z.color+"77"} fontSize={Math.min(4,rw/13)} style={{pointerEvents:"none"}}>{z.w}×{z.h}</text>
          </g>})}
...
function OverlayPanel({type,project,zones,walls,totalCost,netCost,kdvAmt,totalPower,monthlyE,onClose,onStatus,onDiscount}){
  const w={position:"absolute",inset:0,background:"#06080bee",zIndex:10,overflowY:"auto",padding:14};
  const i={maxWidth:560,margin:"0 auto"};const bx={background:"#0c0f16",borderRadius:8,padding:12,border:"1px solid #161a24",marginBottom:8};
  const hd=t=><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><h2 style={{margin:0,fontSize:14,fontWeight:300,color:"#2980B9",letterSpacing:3}}>{t}</h2><button onClick={onClose} style={{background:"none",border:"none",color:"#555",cursor:"pointer",fontSize:16}}>✕</button></div>;
  ...
  if(type==="report")return<div style={w}><div style={i}>{hd("PROJE RAPORU")}
    <div style={bx}><div style={{display:"flex",justifyContent:"space-between"}}><div><div style={{fontSize:7,color:"#444",letterSpacing:2}}>ND GROUP — NOKTA DİZAYN</div><div style={{fontSize:16,fontWeight:600,color:"#e0dcd4",marginTop:2}}>{project.name}</div>{project.customer&&<div style={{fontSize:9,color:"#2980B9"}}>Müşteri: {project.customer}</div>}</div><Logo sz={36}/></div></div>
    {totalCost>0&&<div style={{...bx,textAlign:"center",background:"#2980B911",border:"1px solid #2980B922"}}><div style={{fontSize:8,color:"#2980B9",letterSpacing:2}}>TOPLAM (KDV DAHİL)</div><div style={{fontSize:20,fontWeight:700,color:"#2980B9",marginTop:3}}>₺{fmt(Math.round(netCost+kdvAmt))}</div></div>}
    <div style={bx}>{zones.map(z=><div key={z.id} style={{padding:"5px 0",borderBottom:"1px solid #111418"}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:10,fontWeight:600,color:"#ddd"}}><span style={{color:z.color}}>■</span>{z.icon}{z.label}</span><span style={{fontSize:8,color:"#555"}}>{((z.w*z.h)/10000).toFixed(1)}m²</span></div>{(z.equip||[]).map(eq=><div key={eq.uid} style={{fontSize:8,color:"#666",paddingLeft:10}}>• {eq.brand} {eq.name} — ₺{fmt(eq.price)}</div>)}</div>)}</div>
  </div></div>;

  if(type==="pricing")return<div style={w}><div style={i}>{hd("FİYAT TEKLİFİ")}
    <div style={bx}><table style={{width:"100%",borderCollapse:"collapse",fontSize:9}}><thead><tr style={{borderBottom:"1px solid #1e2530"}}><th style={{textAlign:"left",padding:3,color:"#888"}}>Ürün</th><th style={{textAlign:"left",padding:3,color:"#888"}}>Marka</th><th style={{textAlign:"right",padding:3,color:"#888"}}>Fiyat</th></tr></thead>
      <tbody>{allEq.map((eq,i)=><tr key={eq.uid||i} style={{borderBottom:"1px solid #111418"}}><td style={{padding:3,color:"#ddd"}}>{eq.name}</td><td style={{padding:3,color:eq.color}}>{eq.brand}</td><td style={{padding:3,textAlign:"right",color:"#2980B9"}}>₺{fmt(eq.price)}</td></tr>)}</tbody></table></div>
    <div style={bx}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}><span style={{fontSize:10,color:"#888"}}>İskonto (%)</span><input type="number" value={project.discount||0} onChange={e=>onDiscount(+e.target.value)} style={{width:60,padding:"2px 5px",background:"#080a0e",border:"1px solid #1a1e28",borderRadius:3,color:"#ddd",fontSize:11,textAlign:"right",outline:"none"}}/></div>
  ...
  if(type==="energy")return<div style={w}><div style={i}>{hd("ENERJİ")}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
      <div style={{...bx,textAlign:"center"}}><div style={{fontSize:20,fontWeight:700,color:"#F1C40F"}}>{(totalPower/1000).toFixed(1)}kW</div><div style={{fontSize:8,color:"#555"}}>Toplam Güç</div></div>
      <div style={{...bx,textAlign:"center"}}><div style={{fontSize:20,fontWeight:700,color:"#E67E22"}}>₺{fmt(Math.round(monthlyE))}</div><div style={{fontSize:8,color:"#555"}}>Aylık Tahmini</div></div>
    </div>
    <div style={bx}>{allEq.filter(e=>e.power>0).map((eq,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",fontSize:9,borderBottom:"1px solid #111418"}}><span style={{color:"#ddd"}}>{eq.brand} {eq.name}</span><span style={{color:"#F1C40F"}}>{eq.power}W</span></div>)}</div>
  </div></div>;
  ...
  if(type==="status")return<div style={w}><div style={i}>{hd("PROJE DURUMU")}
    <div style={{display:"flex",gap:3,marginBottom:14,flexWrap:"wrap"}}>
      {STATUS_FLOW.map((s,idx)=>{const cur=STATUS_FLOW.findIndex(x=>x.id===project.status);const isActive=project.status===s.id;const isPast=cur>=idx;return<div key={s.id} style={{textAlign:"center",flex:1,minWidth:50}}>
        <div onClick={()=>onStatus(s.id)} style={{width:30,height:30,borderRadius:15,margin:"0 auto",background:isPast?s.color+"33":"#161a24",border:`2px solid ${isPast?s.color:"#1e2530"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,cursor:"pointer"}}>{s.icon}</div>
        <div style={{fontSize:7,color:isActive?s.color:"#444",marginTop:3,fontWeight:isActive?700:400}}>{s.label}</div>
      </div>})}
    </div>
    <div style={{display:"flex",gap:6,marginBottom:10}}>{[{l:"Sunum",c:"#9B59B6"},{l:"Detay",c:"#2980B9"},{l:"Uygulama",c:"#27AE60"}].map(p=><div key={p.l} style={{flex:1,padding:6,background:p.c+"11",borderRadius:6,textAlign:"center"}}><div style={{fontSize:8,color:p.c,fontWeight:600}}>{p.l}</div><div style={{fontSize:7,color:"#444",marginTop:2}}>{STATUS_FLOW.filter(s=>s.phase===p.l.toLowerCase()).map(s=>s.label).join(" → ")}</div></div>)}</div>
    {project.approvedAt&&<div style={{...bx,textAlign:"center"}}><div style={{fontSize:12,color:"#27AE60"}}>✅ Konsept Onaylandı</div><div style={{fontSize:9,color:"#666",marginTop:2}}>{new Date(project.approvedAt).toLocaleString("tr-TR")}{project.approvedBy&&` — ${project.approvedBy}`}</div></div>}
    {(project.versions||[]).length>0&&<div style={bx}><div style={{fontSize:8,color:"#2980B9",letterSpacing:2,marginBottom:4}}>VERSİYONLAR</div>{(project.versions||[]).map((v,i)=><div key={v.id} style={{fontSize:9,color:"#888",padding:"2px 0",borderBottom:"1px solid #111418"}}>Rev.{i+1} — {new Date(v.date).toLocaleString("tr-TR")}</div>)}</div>}
  </div></div>;
...
// ─── 3D & FIELD VIEWS ───────────────────────────────────────────
function View3D({project}){return<div style={{flex:1,overflow:"auto",background:"#06080b",padding:16,display:"flex",justifyContent:"center"}}><div style={{textAlign:"center"}}><div style={{fontSize:12,color:"#2980B9",letterSpacing:2,marginBottom:10}}>3D KUŞ BAKIŞI — {project.name}</div><div style={{background:"#0a0c10",borderRadius:10,border:"1px solid #161a24",padding:8}}><Iso3D project={project}/></div></div></div>}

function FieldView({project}){
  const zones=project.zones||[];
  return<div style={{flex:1,overflow:"auto",background:"#06080b",padding:16}}><div style={{maxWidth:600,margin:"0 auto"}}>
    <div style={{fontSize:12,color:"#2980B9",letterSpacing:2,marginBottom:4}}>🔧 SAHA GÖRÜNÜMÜ</div>
    <div style={{fontSize:9,color:"#555",marginBottom:12}}>{project.name}{project.customer&&` — ${project.customer}`}</div>
    {zones.map(z=><div key={z.id} style={{background:"#0c0f16",borderRadius:8,padding:12,border:`1px solid ${z.color}33`,marginBottom:8}}>
      <div style={{fontSize:12,fontWeight:600,color:z.color,marginBottom:4}}>{z.icon}{z.label}</div>
      <div style={{fontSize:9,color:"#555",marginBottom:4}}>({z.x},{z.y}) — {z.w}×{z.h}cm — {((z.w*z.h)/10000).toFixed(1)}m²</div>
      {(z.equip||[]).length>0&&<table style={{width:"100%",borderCollapse:"collapse",fontSize:9}}><thead><tr style={{borderBottom:"1px solid #1e2530"}}><th style={{textAlign:"left",padding:2,color:"#666"}}>Ekipman</th><th style={{textAlign:"left",padding:2,color:"#666"}}>Model</th><th style={{textAlign:"center",padding:2,color:"#666"}}>✓</th></tr></thead>
        <tbody>{(z.equip||[]).map(eq=><tr key={eq.uid} style={{borderBottom:"1px solid #111418"}}><td style={{padding:2,color:"#ddd"}}>{eq.brand} {eq.name}</td><td style={{padding:2,color:"#888"}}>{eq.model}</td><td style={{padding:2,textAlign:"center"}}>☐</td></tr>)}</tbody></table>}
      {z.notes&&<div style={{fontSize:8,color:"#666",marginTop:3,fontStyle:"italic"}}>📝 {z.notes}</div>}
    </div>)}
  </div></div>;
}
